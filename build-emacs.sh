...
input="$1"
_usage() { echo "Usage: ./build-emacs.sh --x11|--nox"; }

if [[ ${input/--/} == nox || ${input/--/} == x11 ]]; then

    # update the system and install dependancies
    sudo bash -c 'dnf upgrade -y && dnf install -y lcms2-devel gcc-c++ && dnf builddep -y emacs'
